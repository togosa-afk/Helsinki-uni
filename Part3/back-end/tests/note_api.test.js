...
describe('when there is initially some notes saved', () => {
  ...
  describe('addition of a new note', () => {
    ...
    test('fails with status code 400 if data invalid', async () => {
      const newNote = { important: true, userId }
    ...
    test('created note is linked to the user', async () => {
      const newNote = {
        content: 'note linked to user',
        important: false,
        userId,
      }
      ...
      const noteIds = userAtEnd.notes.map(id => id.toString())

      assert(noteIds.includes(created.body.id))
    })
  })
